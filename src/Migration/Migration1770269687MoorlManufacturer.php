...
class Migration1770269687MoorlManufacturer extends MigrationStep
{
    public const OPERATION_HASH = 'eb4ea9b39bf879209e0639d0b86a2c60';
    ...
    {
        $sql = <<<SQL
CREATE TABLE moorl_manufacturer (id BINARY(16) NOT NULL, avatar_id BINARY(16) DEFAULT NULL, banner_id BINARY(16) DEFAULT NULL, cms_page_id BINARY(16) DEFAULT NULL, country_id BINARY(16) DEFAULT NULL, country_state_id BINARY(16) DEFAULT NULL, media_id BINARY(16) DEFAULT NULL, product_manufacturer_id BINARY(16) NOT NULL, cms_page_version_id BINARY(16) DEFAULT 0x0FA91CE3E96A4BC2BE4BD9CE752C3425, product_manufacturer_version_id BINARY(16) DEFAULT 0x0FA91CE3E96A4BC2BE4BD9CE752C3425 NOT NULL, active TINYINT(1) DEFAULT 0, product_count INT DEFAULT NULL, additional_address_line1 VARCHAR(255) DEFAULT NULL, additional_address_line2 VARCHAR(255) DEFAULT NULL, banner_color VARCHAR(255) DEFAULT NULL, city VARCHAR(255) DEFAULT NULL, country_code VARCHAR(255) DEFAULT NULL, email VARCHAR(255) DEFAULT NULL, location_place_id VARCHAR(255) DEFAULT NULL, merchant_url VARCHAR(255) DEFAULT NULL, phone_number VARCHAR(255) DEFAULT NULL, shop_url VARCHAR(255) DEFAULT NULL, street VARCHAR(255) DEFAULT NULL, street_number VARCHAR(255) DEFAULT NULL, zipcode VARCHAR(255) DEFAULT NULL, created_at DATETIME NOT NULL, updated_at DATETIME DEFAULT NULL, PRIMARY KEY (id, product_manufacturer_version_id)) DEFAULT CHARACTER SET utf8mb4;
...
ALTER TABLE moorl_manufacturer ADD CONSTRAINT `fk.moorl_manufacturer.banner_id` FOREIGN KEY (banner_id) REFERENCES media (id) ON UPDATE CASCADE ON DELETE SET NULL;
ALTER TABLE moorl_manufacturer ADD CONSTRAINT `fk.moorl_manufacturer.avatar_id` FOREIGN KEY (avatar_id) REFERENCES media (id) ON UPDATE CASCADE ON DELETE SET NULL;
ALTER TABLE moorl_manufacturer ADD CONSTRAINT `fk.moorl_manufacturer.media_id` FOREIGN KEY (media_id) REFERENCES media (id) ON UPDATE CASCADE ON DELETE SET NULL;
...
SQL;
        ...
        if (!EntityDefinitionQueryHelper::constraintExists($connection, 'moorl_manufacturer', 'fk.moorl_manufacturer.media_id')) {
            $sql = "ALTER TABLE moorl_manufacturer ADD CONSTRAINT `fk.moorl_manufacturer.media_id` FOREIGN KEY (media_id) REFERENCES media (id) ON UPDATE CASCADE ON DELETE SET NULL;";
            EntityDefinitionQueryHelper::tryExecuteStatement($connection, $sql, 'moorl_manufacturer');
        }
